...
class AdjectivalAdverbialMiddleFinalExtractor(ExtractionRule):
    def __init__(self):
        super().__init__()

    def extract(self, leaf: Leaf) -> Extraction:
        p = "ROOT <<: (S < VP=vp & << (/,/=comma1 $+ (ADJP|ADVP=ad ?$+ /,/=comma2)))"
        matches = config.corenlp_client.tregex(leaf.text, p)

        if len(matches['sentences'][0]) > 0:
            match = matches['sentences'][0][str(0)]
            matched_node_ad = tree_utils.matcher_get_node_from_tree(leaf.parse_tree, match, "ad")
            ...
            # rephrase
            left_constituent_words = self.rephrase_enablement(matched_node_ad, matched_node_vp)
            ...
            if matched_node_comma2 is not None:
                ...
                right_constituent_words.extend(parse_tree_extraction_utils.get_following_words(
                    leaf.parse_tree, matched_node_ad, False))

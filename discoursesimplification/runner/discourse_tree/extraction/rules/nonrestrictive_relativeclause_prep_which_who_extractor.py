...
class NonRestrictiveRelativeClausePrepWhichWhoExtractor(ExtractionRule):
    def __init__(self):
        super().__init__()

    def extract(self, leaf: Leaf) -> Extraction:
        p = "ROOT <<: (S << (NP=head <, NP=np & < (/,/=comma $+ (SBAR=sbar <, (WHPP $+ S=s & <, IN=prep & <- WHNP) & ?$+ /,/=comma2))))"
        ...
        if len(matches['sentences'][0]) > 0:
            ...
            right_constituent_words.extend(parse_tree_extraction_utils.get_words_in_between(
                leaf.parse_tree, matched_node_np, matched_node_comma, True, False))

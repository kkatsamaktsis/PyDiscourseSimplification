...
class SubordinationPostAttributionExtractor(ExtractionRule):
    def __init__(self):
        super().__init__()

    def extract(self, leaf: Leaf) -> Extraction:
        p = "ROOT <<: (S < (NP $.. (VP=vp <+(VP) (SBAR=sbar))))"
        ...
        if len(matches['sentences'][0]) > 0:
            ...
            right_constituent_words = parse_tree_extraction_utils.get_containing_words(matched_node_sbar)

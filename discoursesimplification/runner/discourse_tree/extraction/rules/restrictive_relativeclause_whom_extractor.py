...
class RestrictiveRelativeClauseWhomExtractor(ExtractionRule):
    def __init__(self):
        super().__init__()

    def extract(self, leaf: Leaf) -> Extraction:
        p = "ROOT <<: (S << (NP=head <, NP=np & < (SBAR=sbar <, (WHNP $+ (S=s <, NP=np2 & <- (VP=vp ?<+(VP) PP=prep)) & <<: (WP <: whom)))))"
        ...
        if len(matches['sentences'][0]) > 0:
            ...
            if matched_node_prep is not None \
                    and tree_utils.get_value(matched_node_prep).split(" ")[0] == "PP":

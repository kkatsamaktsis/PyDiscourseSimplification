...
class PrepositionalAttachedtoVPExtractor(ExtractionRule):
    def __init__(self):
        super().__init__()

    def extract(self, leaf: Leaf) -> Extraction:
        p = "ROOT <<: (S <+(S|VP) (VP < (PP=prep $- NP|PP)) & < VP=vp)"
        ...
        if len(matches['sentences'][0]) > 0:
            ...
            right_constituent_words.extend(parse_tree_extraction_utils.get_preceding_words(leaf.parse_tree,
                                                                                           matched_node_prep, False))
            right_constituent_words.extend(parse_tree_extraction_utils.get_following_words(leaf.parse_tree,
                                                                                           matched_node_prep, False))

...
class NonRestrictiveRelativeClauseWhoseExtractor(ExtractionRule):
    def __init__(self):
        super().__init__()

    def extract(self, leaf: Leaf) -> Extraction:
        p = "ROOT <<: (S << (NP=head < (NP=np $+ (/,/=comma $+ (SBAR=sbar <, (WHNP $+ S=s & <, (/WP\\$/ $+ /.*/=nn)) & ?$+ /,/=comma2)))))"
        ...
        if len(matches['sentences'][0]) > 0:
            ...
            matched_node_nn = tree_utils.matcher_get_node_from_tree(leaf.parse_tree, match, "nn")
            ...
            if tree_utils.get_value(tree_utils.first_child(matched_node_s)).split(" ")[0] == "NP":
                ...
                right_constituent_words.append("'s")
                right_constituent_words.extend(parse_tree_extraction_utils.get_containing_words(matched_node_nn))
            else:
                right_constituent_words.extend(parse_tree_extraction_utils.get_containing_words(matched_node_np))
                right_constituent_words.append("'s")
                right_constituent_words.extend(parse_tree_extraction_utils.get_containing_words(matched_node_nn))
            ...
            # relation cue_phrase_words = parse_tree_extraction_utils.get_preceding_words(matched_node_sbar,
            # matched_node_s, False)
            # relation = self.classifier.classify_subordinating(cue_phrase_words) if relation
            # is None: relation = Relation.DESCRIBING_DEFINITION

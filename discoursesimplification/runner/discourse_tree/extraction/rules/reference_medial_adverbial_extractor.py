...
class ReferenceMedialAdverbialExtractor(ExtractionRule):
    def __init__(self):
        super().__init__()

    def extract(self, leaf: Leaf) -> Extraction:
        p = "ROOT <<: (S < (ADVP|PP=adv , /,/=begin . /,/=end $,, NP $.. VP))"
        ...
        if len(matches['sentences'][0]) > 0:  # Should be loop
            ...
            matched_node_begin = tree_utils.matcher_get_node_from_tree(leaf.parse_tree, match, "begin")
            ...
            words = []
            words.extend(parse_tree_extraction_utils.get_preceding_words(leaf.parse_tree, matched_node_begin, False))
            words.extend(parse_tree_extraction_utils.get_following_words(leaf.parse_tree, matched_node_end, False))

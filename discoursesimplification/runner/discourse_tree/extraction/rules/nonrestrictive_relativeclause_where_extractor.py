...
class NonRestrictiveRelativeClauseWhereExtractor(ExtractionRule):
    def __init__(self):
        super().__init__()

    def extract(self, leaf: Leaf) -> Extraction:
        p = "ROOT <<: (S << (/.*/=head < (NP|PP $+ (/,/=comma $+ (SBAR=sbar <, (WHADVP $+ S=s & <<: WRB) & ?$+ /,/=comma2)))))"
        ...
        if len(matches['sentences'][0]) > 0:
            ...
            # matched_node_s, False) relation = self.classifier.classify_subordinating(cue_phrase_words)
            # if relation is None:

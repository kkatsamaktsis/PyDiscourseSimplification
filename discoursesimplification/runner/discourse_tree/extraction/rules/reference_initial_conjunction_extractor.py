...
class ReferenceInitialConjunctionExtractor(ExtractionRule):
    def __init__(self):
        super().__init__()
    ...
    def extract(self, leaf: Leaf) -> Extraction:
        ...
        if len(matches['sentences'][0]) > 0:  # Should be loop
            ...
            matched_node_cc = tree_utils.matcher_get_node_from_tree(leaf.parse_tree, match, "cc")

            cue_phrase_words = parse_tree_extraction_utils.get_containing_words(matched_node_cc)

            # the right constituent
            words = []
            words.extend(parse_tree_extraction_utils.get_preceding_words(leaf.parse_tree, matched_node_cc, False))
            words.extend(parse_tree_extraction_utils.get_following_words(leaf.parse_tree, matched_node_cc, False))
